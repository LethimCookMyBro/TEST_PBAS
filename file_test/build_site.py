...
summaries["mock"] += '<li style="margin-bottom:15px"><div style="margin:10px 0"><img src="images/ch10/page_09.png" style="max-height:150px; border-radius:4px" alt="broadcast"></div><b>ปัญหา Broadcast Storm มักเกิดขึ้นในอุปกรณ์ใดเมื่อไม่มีการป้องกันลูปสายแลน?</b><br><span style="color:var(--yw)">ตอบ:</span> Switch (ต้องแก้ด้วยการเปิดใช้โหมด Spanning-Tree)</li>'
...
summaries["mock"] += '<li style="margin-bottom:15px"><div style="margin:10px 0"><img src="images/ch9/page_12.png" style="max-height:150px; border-radius:4px" alt="roas"></div><b>จากภาพของการทำ Inter-VLAN คำสั่งใดต้องพิมพ์บน interface ย่อย (เช่น 0/0/0.10) ของ Router เสมอ?</b><br><span style="color:var(--yw)">ตอบ:</span> <code class="hl">encapsulation dot1Q [VLAN-ID]</code></li>'
...
summaries["mock"] += '<li style="margin-bottom:15px"><b>ในระบบ Spanning Tree ค่า Cost หมายเลข 4 หมายถึงพอร์ตสายแลนความเร็วเท่าใด?</b><br><span style="color:var(--yw)">ตอบ:</span> 1 Gbps (Gigabit Ethernet)</li>'
...
summaries["mock"] += '<li style="margin-bottom:15px"><div style="margin:10px 0"><img src="images/ch11/page_09.png" style="max-height:150px; border-radius:4px" alt="lacp"></div><b>(อาจารย์ใบ้โหมด) การทำ EtherChannel รูปแบบ LACP ต้องใช้การตั้งค่าโหมดลิงก์ว่าอย่างไร?</b><br><span style="color:var(--yw)">ตอบ:</span> ใช้การจับคู่แบบ <code class="hl">active</code> คู่กับ <code class="hl">active</code> (หรือคู่กับ passive)</li>'
...
summaries["mock"] += '<li style="margin-bottom:15px"><div style="margin:10px 0"><img src="images/ch10new/page_12.png" style="max-height:150px; border-radius:4px" alt="acl mask"></div><b>(คำนวณ 1 ข้อ) 若ต้องการกำหนดช่วง Network 10.10.10.0/24 ในคำสั่ง ACL จะต้องเว้นวรรคและตามด้วย Wildcard Mask ค่าเลขใด?</b><br><span style="color:var(--yw)">ตอบ:</span> <code class="hl">0.0.0.255</code> (กลับหัวขั้วตรงข้ามกับ Subnet Mask 255.255.255.0)</li>'

# IPv6 & Miscellaneous (to hit exactly 30 Qs based on broad hints of chapter scope)
summaries["mock"] += '<li style="margin-bottom:15px"><b>(IPv6) หากต้องการเขียนย่อเลขหมาย <code class="hl">2001:0db8:0000:0000:0000:0000:0000:0001</code> ควรย่ออย่างไร?</b><br><span style="color:var(--yw)">ตอบ:</span> <code class="hl">2001:db8::1</code></li>'
...
summaries["mock"] += '<li style="margin-bottom:15px"><b>(คำนวณ) หากสวิตช์ 2 ตัวต่อสายแลนคู่ขนานกัน เกิดอาการกระพริบไม่หยุดแล้วเน็ตค้าง นั่นเรียกว่าอาการอะไรที่ต้องแก้ด้วย STP?</b><br><span style="color:var(--yw)">ตอบ:</span> เกิดอาการ Broadcast Storm</li>'
